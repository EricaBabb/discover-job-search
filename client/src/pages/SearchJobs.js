...
=======
import { SAVE_JOB } from '../utils/mutation';
import { useMutation, useQuery } from '@apollo/client';

import Auth from '../utils/auth';
import { saveJob, searchIndeed } from '../util/api';
import { saveJobIds, getSavedJobIds } from '../utils/localStorage';

>>>>>>> afdf2e787f3905560d4e1bd861c99aa3077d877a
const SearchJobs = () => {
...
=======
    ...
    const handleFormSubmit = async (event) => {
        ...
        try {
            const response = await searchIndeedJobs(searchInput);
                    ...
                    {searchedJobs.map((job) => {
                        return (
                            <Card key={job.jobId} border='dark'>
                                ...
                                <Card.Body>
                                    <Card.Title>{job.title}</Card.Title>
                                    <p className='small'>Jobs: {job.authors}</p>
                                    <Card.Text>{job.description}</Card.Text>
                                    {Auth.loggedIn() && (
                                        <Button
                                            disabled={savedJobIds?.some((savedJobId) => savedJobId === job.jobId)}
